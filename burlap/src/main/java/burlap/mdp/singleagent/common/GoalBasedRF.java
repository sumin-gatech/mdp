...
public class GoalBasedRF implements RewardFunction {
	...
	protected GridWorldState rs;
	protected double rsReward;
	protected boolean is_rs=false;
	...
	public double reward(State s, Action a, State sprime) {
		...
		if (this.is_rs){
			GridWorldState ss=(GridWorldState)sprime;
			if(ss.agent.x==this.rs.agent.x && ss.agent.y==this.rs.agent.y)
				return this.rsReward;
		}
		
		return defaultReward;
	}

	public void setReward(GridWorldState s, double reward){
		this.is_rs=true;
		this.rs=s;
		this.rsReward=reward;
	}
}
